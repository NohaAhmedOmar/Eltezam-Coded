    ...
    $.ajax({
        ...
        success: function (results) {
            ...
            $.each(results.data, function (i, result) {
                ...
                Employees.push({
                    id: result.id, name: result.name, jobNumber: result.jobNumber, jobClassCode: result.jobClassCode, jobNameCode: result.jobNameCode,
                    rankCode: result.rankCode, startDate: result.startDate, employeeId: result.employeeId, employmentTypeCode: result.employmentTypeCode,
                    positionOrganizationID: result.positionOrganizationID, positionOrganizationName: result.positionOrganizationName,
                    jobClassCodeKey: result.jobClassCodeKey, jobNameCodeKey: result.jobNameCodeKey, locationCode: result.locationCode, lastUpdateDate: result.lastUpdateDate, positionStatus: result.positionStatus
                })
...
function SubmitSheet() {
    if (document.getElementById("sheet").files[0] != null) {
        ...
        $.ajax({
            type: "Post",
            url: '/api/UploadSheets/PostJobsExcelSheet',
            data: Sheet,
            processData: false,
            contentType: false,
...
function sendToService(id) {
    ...
    if (checkeddata.length != 0) {
        $.ajax({
            type: "Post",
            url: "/api/SendToService/SendJobInfoToService",
    ...
    if (selectedToDelete.length != 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "Post",
                    url: "/api/Employees/BulkDeleteJob",
