...
function GetEmployees() {
    ...
    $.ajax({
        type: "Get",
        url: '/api/Employees/GetEmployeesVacationsView',
        success: function (results) {
            ...
            $.each(results.data, function (i, result) {
                ...
                Employees.push({ id: result.id, employeeId: result.employeeId, name: result.name, vacationCode: result.vacationCode, period: result.period, startDate: result.startDate, endDate: result.endDate, vacationCodeKey: result.vacationCodeKey })
...
function SubmitSheet() {
    if (document.getElementById("sheet").files[0] != null) {
        ...
        $.ajax({
            type: "Post",
            url: '/api/UploadSheets/PostEmployeeVacationsExcelSheet',
            data: Sheet,
            processData: false,
            contentType: false,
...
function sendToService(id) {
    ...
    if (checkeddata.length != 0) {
        $.ajax({
            type: "Post",
            url: "/api/SendToService/SendVacationInfoToService",
    ...
    if (selectedToDelete.length != 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "Post",
                    url: "/api/Employees/BulkDeleteEmployeeVacation",
