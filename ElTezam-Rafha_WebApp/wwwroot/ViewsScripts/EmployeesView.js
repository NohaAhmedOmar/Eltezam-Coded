...
function GetEmployees() {
    ...
    $.ajax({
        type: "Get",
        url: '/api/Employees/GetEmployeesView',
        success: function (results) {
            ...
            $.each(results.data, function (i, result) {
                ...
                Employees.push({
                    id: result.id, name: result.name, birthDate: result.birthDate, nationality: result.nationality, jobNameCode: result.jobNameCode,
                    rankCode: result.rankCode, transactionCode: result.transactionCode,
                    jobNameCodeKey: result.jobNameCodeKey, jobClassCodeKey: result.jobClassCodeKey, transactionCodeKey: result.transactionCodeKey,
                    employmentTypeCodeKey: result.employmentTypeCodeKey, firstGradeDate: result.firstGradeDate, jobOrganizationId: result.jobOrganizationId,
                    jobOrganizationName: result.jobOrganizationName, basicSalary: result.basicSalary, locationCode: result.locationCode,
                    ministryHireDate: result.ministryHireDate, isActive: result.isActive, lastUpdateDate: result.lastUpdateDate, healhStatus: result.healhStatus,
                    gender: result.gender, maritalStatus: result.maritalStatus, rankCodeKey: result.rankCodeKey, employeeStatusCode: result.employeeStatusCode, nationalID: result.nationalID
                    , stepId: result.stepId, religion: result.religion
                })
...
function SubmitSheet() {
    if (document.getElementById("sheet").files[0] != null) {
        ...
        $.ajax({
            type: "Post",
            url: '/api/UploadSheets/PostEmployeesExcelSheet',
            data: Sheet,
            processData: false,
            contentType: false,
        ...
        ValidationAlert('لابد من اختيار الملف')
...
function sendToService(id) {
    ...
    if (checkeddata.length != 0) {
        $.ajax({
            type: "Post",
            url: "/api/SendToService/SendEmployeeToService",
    ...
    if (selectedToDelete.length != 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "Post",
                    url: "/api/Employees/BulkDeleteEmployees",
