...
function SubmitSheet() {
    if (document.getElementById("sheet").files[0] != null) {
        ...
        $.ajax({
            type: "Post",
            url: '/api/UploadSheets/PostEmployeeAppraisalsExcelSheet',
            data: Sheet,
            processData: false,
            contentType: false,
...
function sendToService(id) {
    ...
    if (checkeddata.length != 0) {
        $.ajax({
            type: "Post",
            url: "/api/SendToService/SendAppraisalToService",
    ...
    if (selectedToDelete.length != 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "Post",
                    url: "/api/Employees/BulkDeleteEmployeeAppraisal",
