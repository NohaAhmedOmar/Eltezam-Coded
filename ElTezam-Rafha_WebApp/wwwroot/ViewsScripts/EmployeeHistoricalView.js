...
function GetEmployees() {
    ...
    $.ajax({
        type: "Get",
        url: '/api/Employees/GetEmployeeHistoricalView',
        success: function (results) {
            $.each(results.data, function (i, result) {
                Employees.push({
                    id: result.id,
                    name: result.name, jobNumber: result.jobNumber, jobClassCode: result.jobClassCode, jobNameCode: result.jobNameCode,
...
function SubmitSheet() {
    if (document.getElementById("sheet").files[0] != null) {
        ...
        $.ajax({
            type: "Post",
            url: '/api/UploadSheets/PostEmloyeeHistoricalInfoExcelSheet',
            data: Sheet,
            processData: false,
            contentType: false,
...
function sendToService(id) {
    ...
    if (checkeddata.length != 0) {
        $.ajax({
            type: "Post",
            url: "/api/SendToService/SendHistoricalInfoToService",
    ...
    if (selectedToDelete.length != 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "Post",
                    url: "/api/Employees/BulkDeleteEmployeeHistorical",
