...
function GetEmployees() {
    $.ajax({
        type: "Get",
        url: '/api/Employees/GetEmployeesQualificationsView',
        success: function (results) {
            $.each(results.data, function (i, result) {
                Employees.push({
                    id: result.id, name: result.name, majorCode: result.majorCode, universityName: result.universityName,
                    qualificationStatus: result.qualificationStatus, qualificationCode: result.qualificationCode,
                    nationalID: result.nationalID, qualificationCodeKey: result.qualificationCodeKey, majorCodeKey: result.majorCodeKey,
                    universityCode: result.universityCode, transactionType: result.transactionType
                })
            });
...
function SubmitSheet() {
    if (document.getElementById("sheet").files[0] != null) {
        ...
        $.ajax({
            type: "Post",
            url: '/api/UploadSheets/PostEmployeeQualificationsExcelSheet',
            data: Sheet,
            processData: false,
            contentType: false,
...
function sendToService(id) {
    ...
    if (checkeddata.length != 0) {
        $.ajax({
            type: "Post",
            url: "/api/SendToService/SendQualificationInfoToService",
    ...
    if (selectedToDelete.length != 0) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "Post",
                    url: "/api/Employees/BulkDeleteEmployeeQualification",
